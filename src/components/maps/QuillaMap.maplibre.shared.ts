...
export const DARK_MAP_THEME = {
  background: '#243246',
  controlBackground: '#151E2D',
  controlBorder: '#3B4D68',
  controlText: '#F8D84A',
};
  ...
  layers: [
    {
      id: 'dark-background',
      type: 'background',
      paint: { 'background-color': DARK_MAP_THEME.background },
    },
    {
      id: 'carto-dark',
      type: 'raster',
...
export const getShadeZonesFeatureCollection = (zones: QuillaMapShadeZone[]) => ({
  ...
  features: zones.map((zone) => ({
    type: 'Feature' as const,
    id: zone.id,
    properties: { id: zone.id, title: zone.title, radiusMeters: zone.radiusMeters },
...
export const getCoordinateFeatureCollection = (
  ...
  type: 'FeatureCollection' as const,
  features: coordinate
    ? [{ type: 'Feature' as const, id, properties: { id }, geometry: pointGeometry(coordinate) }]
    : [],
});
