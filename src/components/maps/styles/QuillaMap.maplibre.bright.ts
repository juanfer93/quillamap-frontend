...
import type { QuillaMapCoordinate, QuillaMapRoutePoint, QuillaMapShadeZone } from '../types/QuillaMap.types';
...
export const getBuildingsFeatureCollection = (places: PlaceMapFeature[]) => ({ type: 'FeatureCollection' as const, features: places.flatMap((place) => {
  ...
  return polygon && height ? [{ type: 'Feature' as const, id: `building-${place.id}`, properties: { id: place.id, source: place.source, color: place.source === 'tourist_site' ? PLACES_VISUAL_IDENTITY.sandGold.hex : PLACES_VISUAL_IDENTITY.sharkBlue.hex, height, base: place.metadata?.extrusionBaseMeters ?? 0 }, geometry: polygon }] : [];
}) });
